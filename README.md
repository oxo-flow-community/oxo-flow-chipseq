...
Requires **oxo-flow >= 0.17.0**. The recommended route is the release binary:
...
oxo-flow dry-run main.oxoflow       # prints the 213-instance plan (154 run, 59 gated off)
...
| DUMP_SOFTWARE_VERSIONS | engine-native export: `oxo-flow report --versions-yml <file> main.oxoflow` | — | oxo-flow ≥ 0.17.0 exports an nf-core-style `versions.yml` derived statically from the workflow declarations: one entry per rule (84 rules) with the pinned container image (registry + tag). Deviation: it is a standalone CI-diff artifact, not a per-process runtime capture — upstream records each tool's runtime version at execution time; the export reflects the pinned versions in the definition (resolved runtime package versions depend on the execution environment). Per-rule `versions.yml` emission inside every command is deliberately not replicated (it would change every rule's command while the default plan stays byte-identical). |
| pipeline summary + software versions sections of MultiQC | `multiqc_workflow_summary` (scripts/multiqc_workflow_summary.py) | Python 3 | port of the nf-core template paramsSummaryMap + paramsSummaryMultiqc pair: renders the Workflow Summary custom-content YAML (per-schema-group params that differ from defaults + Core options adapted to the engine: version / commandLine / launchDir / projectDir) into `results/multiqc/workflow_summary_mqc.yaml`, which all four MultiQC rules consume (MultiQC auto-discovers it from the run root). (The software-versions half — DUMP_SOFTWARE_VERSIONS — is covered by the engine-native export above.) |
